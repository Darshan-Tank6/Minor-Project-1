          ...
          msg.on("body", (stream) => {
            simpleParser(stream, async (err, parsed) => {
              ...
              if (isBotCCed && !isReply) {
                const cleanText = cleanEmailText(parsed);
                const extractedObj = await extractEntities(cleanText);

                let extracted = {
                  student_email: from,
                  roll_no: extractedObj.ROLL_NO || "",
                  department: extractedObj.DEPARTMENT || "",
                  reason: extractedObj.REASON || "",
                  from_date: extractedObj.FROM_DATE || "",
                  to_date: extractedObj.TO_DATE || "",
                  teacher_emails: toList,
                  approved_by: [],
...
module.exports = { startEmailReader };
